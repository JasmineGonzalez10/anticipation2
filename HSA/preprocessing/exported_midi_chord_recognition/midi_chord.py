...
from HSA.preprocessing.exported_midi_chord_recognition.extractors.midi_utilities import is_percussive_channel
...
class ChordRecognition:
    ...
    def process_feature(self,channel_weights):
        ...
        min_subbeat_bass=np.full((n_frame*SUBBEAT_COUNT,),259,dtype=np.intc)
        notes=[]
    ...
    def decode(self):
        ...
        prec=np.zeros((n_frame),dtype=np.intc)
        prei=np.zeros((n_frame),dtype=np.intc)
