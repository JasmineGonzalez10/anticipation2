...
def anticipate(events, controls, delta=DELTA*TIME_RESOLUTION):
    ...
    #if len(controls != 0):
        #excess_count = 0
        #control_excess = controls[0] - CONTROL_OFFSET
    while len(controls) != 0:
        tokens.extend(controls[0:3]) #add distorted controls beyond end of sequence
        controls = controls[3:] #consume distorted controls beyond end of sequence
        #excess_count += 1
    '''print("excess controls: ", excess_count)
    print("latest event: ", event_time)
    print("latest control: ", control_excess)
    print("event time: ", event_time)
    print("control time - delta", control_time - delta)
